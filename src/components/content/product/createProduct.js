  ...
  submitHandler(){
    if(this.state.name === "" && this.state.description === " "){
      ...
      const func = (arrReq, input)=>{
          if(arrReq.filter(content => content.toUpperCase().split(' ') == input.toUpperCase().split(' ')).length !== 0){
              return false
          }
          else{
              return true;
      ...
      <Modal isOpen={this.props.create} className={this.props.className}>
        ...
        <ModalBody>
          ...
          <FormGroup>
            ...
            <Input type="text" name="name"  placeholder="" value={this.state.name}  onChange={this.changeHandler} />
          </FormGroup>
          <FormGroup>
            <Label for="">Description</Label>
            <Input type="text" name="description"  placeholder="" value={this.state.description}  onChange={this.changeHandler} />
          </FormGroup>
        </ModalBody>
        <ModalFooter>
